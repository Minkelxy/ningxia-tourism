...
- 当前交互与发布复核版本为 v0.3.98；首页山河绘图、地图区域入场和选中点位提示共享轻量动效基线，并在减少动效设置下恢复静态绘图；列表筛选输入、全站搜索和主要操作在桌面/移动端保持至少 44px 高；收藏列表、地图控制区、景点卡片、路线详情和行前指南继续共享统一的响应式操作节奏，并由双端 E2E、生产构建、Lighthouse 与 Pages 回退校验共同覆盖。
