...
- 当前交互与发布复核版本为 v0.3.97；站点头部、首页首屏、全屏状态页和离线提示共用响应式头部高度变量；列表筛选输入、全站搜索和主要操作在桌面/移动端保持至少 44px 高；收藏列表、地图控制区、景点卡片、路线详情和行前指南继续共享统一的响应式操作节奏，并由双端 E2E、生产构建、Lighthouse 与 Pages 回退校验共同覆盖。
