...
2026-09-01 线上验收：提交 `b394ddf` 的校验与 Pages 部署 job 均为 **success**；全站搜索框标签结构、聚焦反馈与窄屏结果摘要布局，以及地图纯展示图层移除无动作 Tab 焦点已通过本地与线上构建校验，移动端当前导航语义、首页地图锚点偏移、手记画廊尺寸提示，以及地图预览焦点回退与滚动隔离继续通过回归。
