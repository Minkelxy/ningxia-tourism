...
2026-09-04 v0.3.133 发布复核：最终代码提交 [`5fdb349`](https://github.com/Minkelxy/ningxia-tourism/commit/5fdb349c7ae1845510e4e15731bf2211b999555c) 的[最终主校验与 Pages 部署工作流](https://github.com/Minkelxy/ningxia-tourism/actions/runs/33803123043)为 **success**，build 与 deploy 两个 job 均成功；180 条单元测试、194 条 E2E 通过、16 条按端型跳过，Lighthouse 质量门禁通过且无 flaky。移动端导航菜单与页脚当前入口增加统一激活墨线，与桌面主导航共享纸面状态反馈语言；保留导航路径、当前页语义、菜单焦点与滚动行为，减少动效设置下恢复静态显示。生产构建、73 个页面 sitemap、深链回退与 Pages 部署均通过，Pages 状态为 built。线上根路径、CSS 与 JS 资源 HTTP 200。
