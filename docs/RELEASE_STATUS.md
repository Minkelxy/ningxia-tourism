...
2026-09-23 v0.3.139 发布复核：代码提交 [`05ecca3`](https://github.com/Minkelxy/ningxia-tourism/commit/05ecca3a65c53199cb548445cdb23e87b00d75a0) 的[主校验与 Pages 部署工作流](https://github.com/Minkelxy/ningxia-tourism/actions/runs/35810492272)为 **success**，build（4m1s）与 deploy（10s）两个 job 均成功；197 条单元测试（32 个测试文件）、218 条 E2E 通过、16 条按端型跳过，无 flaky；Lighthouse 质量门禁通过（首页 94、景点 97、手记 94、指南 98；可访问性、最佳实践和 SEO 均为 100）。修正独立路书页的图片导出竞态：省界底图是异步加载的，原先点击「生成图片」即序列化海报，会在省界到达前导出只有路线、没有地图底图的残缺海报且不给提示；现与路线详情页同构，等省界落定后的那一轮渲染再序列化。同时移除路书地理图中永不生效的兜底配色分支、把海报折行的 React key 改为索引，并补一条端到端断言守住导出竞态的回归。仓库另新增 `.gitattributes`（`* text=auto eol=lf`）并把全部已跟踪文件的行尾统一为 LF，消除编辑器改写文件时整体翻动行尾造成的提交噪音，该规整不含任何内容变更。生产构建、82 个页面 sitemap、深链回退与 Pages 部署均通过，Pages 状态为 built。线上根路径、CSS 与 JS 资源 HTTP 200。
...
2026-09-04 v0.3.128 发布复核：最终代码提交 [`401a5a3`](https://github.com/Minkelxy/ningxia-tourism/commit/401a5a3319e96dd17d5a958549788a38bbd351c5) 的[主校验与 Pages 部署工作流](https://github.com/Minkelxy/ningxia-tourism/actions/runs/33792397675)为 **success**，build 与 deploy 两个 job 均成功；180 条单元测试、190 条 E2E 通过、16 条按端型跳过，Lighthouse 质量门禁通过且无 flaky。地图底图新增 5 条低对比度等高线纸面绘图，按地形簇错峰落笔并随地图视口移动；作为非交互装饰补足底图层次，减少动效设置下恢复静态等高线。生产构建、73 个页面 sitemap、深链回退与 Pages 部署均通过，Pages 状态为 built。线上根路径 HTTP 200。
...
2026-09-03 v0.3.120 发布复核：最终代码提交 [`e4a09f9`](https://github.com/Minkelxy/ningxia-tourism/commit/e4a09f97c799782aea63db71d559aa21984c15fd) 的[主校验与 Pages 部署工作流](https://github.com/Minkelxy/ningxia-tourism/actions/runs/33774855974)为 **success**，build 与 deploy 两个 job 均成功；180 条单元测试、179 条 E2E 通过、15 条按端型跳过，Lighthouse 质量门禁通过且无 flaky。旅行手记切换栏目、城市、标签或搜索词时，结果摘要、文章卡片和空结果按新的筛选结果重新挂载并渐进绘图，让内容筛选反馈与路线结果保持统一节奏；保留 URL 同步、栏目键盘切换、清空筛选与文章链接，减少动效设置下恢复静态结果。生产构建、73 个页面 sitemap、深链回退与 Pages 部署均通过，Pages 状态为 built。线上根路径 HTTP 200。
