...
2026-09-03 v0.3.108 发布复核：待 GitHub Actions 完成后回填代码提交、工作流编号与最终测试统计。旅行手记列表使用 `--journal-card-delay` 触发 `journal-card-in` 错峰入场，标题使用 `journal-card-title-ink` 展开墨线；文章详情使用 `journal-detail-photo-in`、`journal-detail-title-in`、`journal-detail-block-in`、`journal-detail-body-in` 和 `journal-sidebar-in` 分层入场；减少动效设置下恢复静态阅读层级与墨线，目录、来源、关联内容和链接行为不变。
