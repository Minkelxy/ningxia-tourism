...
2026-09-03 v0.3.116 发布复核：待 GitHub Actions 完成后回填代码提交、工作流编号与最终测试统计。地图工具栏使用 `map-toolbar-in`、`map-breadcrumb-in`、`map-control-in`、`map-control-pressed`、`map-level-label-in` 和 `map-level-ink` 完成工具栏、层级、控制按钮和图层启用的渐进绘图；减少动效设置下恢复静态工具栏与墨线，地图缩放、重置、层级切换、交通/美食/政府图层和键盘行为不变。
