...
> v0.3.131 发布补充（2026-09-04）：`RouteRecommendation` 的已选筛选按钮通过 `aria-pressed="true"` 触发 `button::after` 的 `route-filter-active-ink`；该伪元素为非交互装饰，不改变筛选、URL、比较表、路线卡片或窄屏布局，`prefers-reduced-motion` 下静态显示。
...
> 当前实现快照已更新至 v0.3.131：首页山河绘图分层入场、页面切换绘图、地图方向罗盘、地图底图等高线、推荐路线筛选选中态墨线、景点兴趣筛选选中态墨线、旅行手记栏目激活墨线、地图叠加图层墨线落印、路线按天导航激活墨线、首页路线结果绘图、地图区域入场、地图视口过渡、路线轨道与节点落印、地图层级图层重绘、各类筛选结果重绘、全站搜索结果重绘、区域聚焦墨线、选中点位墨线和公共反馈层继续使用统一的响应式与动效约束；地图数据、搜索索引和内容核验机制不变。
