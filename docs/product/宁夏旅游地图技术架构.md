...
> 当前实现快照（2026-09-01，v0.3.26）：全站卡片、导航、筛选面板、路线筛选、景点兴趣筛选选中态、次级行动按钮、行前清单、搜索清空、地图图层选中态、地图层级、首页探索、搜索快捷词、阅读资料入口、说明页目录、旅行手记栏目、详情页操作、收藏操作、景点对比操作、深色头图按钮和轻量文字操作已统一视觉反馈；主导航、桌面搜索/收藏入口和移动导航通过 `aria-current="page"` 暴露当前页面，兴趣筛选与地图图层开关通过 `aria-pressed` 暴露状态，主次行动按钮均保留各自主题色层级并使用一致的过渡、抬升和阴影节奏；发布验收记录见 [`docs/RELEASE_STATUS.md`](../RELEASE_STATUS.md)。
