...
> v0.3.134 发布补充（2026-09-04）：`RouteDetail` 使用 `activeDay` 为当前 `.route-day` 与 `.day-marker` 注入 `is-active`，当前 DAY 印章使用 `route-day-marker-active-ink` 重新落印；不改变锚点跳转、IntersectionObserver 滚动识别、键盘访问与打印版布局，`prefers-reduced-motion` 下静态显示。
