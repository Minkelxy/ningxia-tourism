...
> v0.3.72 发布补充（2026-09-02）：`AttractionLayer` 将透明 `.marker-hit` 统一为 `r="38"`，与 `FoodLayer` 共享至少 44px 的实际屏幕触控范围；全局地图热区规则增加 `pointer-events: all`，并由双端 E2E 检查实际 `boundingBox`。
