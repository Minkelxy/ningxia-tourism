...
> 当前实现快照（2026-09-01，v0.3.28）：全站卡片、导航、筛选面板、路线筛选、景点兴趣筛选选中态、次级行动按钮、安静按钮、文字链接、行前清单、搜索清空、地图图层选中态、地图层级、首页探索、搜索快捷词、阅读资料入口、说明页目录、旅行手记栏目、详情页操作、收藏操作、景点对比操作、深色头图按钮和轻量文字操作已统一视觉反馈；主导航、桌面搜索/收藏入口和移动导航通过 `aria-current="page"` 暴露当前页面，兴趣筛选与地图图层开关通过 `aria-pressed` 暴露状态，主次行动按钮均保留各自主题色层级，安静按钮按浅色页面与深色头图环境使用对应阴影，文字链接使用不超过 2px 的轻量位移，并保持一致的过渡节奏；发布验收记录见 [`docs/RELEASE_STATUS.md`](../RELEASE_STATUS.md)。
