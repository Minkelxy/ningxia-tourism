...
> v0.3.139 发布补充（2026-09-23）：`RouteRoadbook` 的图片导出改为与 `RouteDetail` 同构的 `posterFeatures` 门禁——省界 GeoJSON 为异步加载，原先点击即序列化，会在它落定前导出只有路线、没有地图底图的残缺海报；现等省界就绪后的那一轮渲染再调用 `exportSvgToPng`（`setFeatures` 触发的重渲染不会同步反映到 DOM，因此不能在同一次点击里先 await 再序列化）。同时移除 `RouteRoadbookMap` 中恒不触发的兜底配色分支（有坐标的停靠点必然来自已发布景点、必然带 `cityId`），海报折行的 React key 改为索引。仓库另新增 `.gitattributes`（`* text=auto eol=lf`）并把全部已跟踪文件行尾统一为 LF；`RouteStop` 数据模型、`validate.ts` 门禁与内容事实不变。
> v0.3.138 发布补充（2026-09-22）：新增 `src/lib/roadbook.ts` 路书模型（纯函数），复用 `getRouteEvidenceSummary` 与 `src/components/map/projection.ts` 的 `createProjection`／`geometryToPath`／`mergeFeatureBounds`，把停靠点分为有坐标与仅有查询串两类，并对有坐标点做最小跨度扩张；地理缩略图按天分段使用 `roadbook-route-draw`、节点使用 `roadbook-node-in`，流线使用 `roadbook-day-in`／`roadbook-day-ink`／`roadbook-node-row-in`，全部在 `prefers-reduced-motion` 下恢复静态终态。`src/components/roadbook/RouteRoadbookPoster.tsx` 输出自包含静态竖版 SVG（仅内联样式、不含位图、不带入场动画），`src/lib/export-svg.ts` 将其序列化后在 canvas 上按 2 倍绘制并导出 PNG，支持 `navigator.share({ files })` 与下载两条路径；省界 GeoJSON 仍为按需懒加载，`RouteStop` 数据模型、`validate.ts` 门禁与内容事实不变。
> v0.3.137 发布补充（2026-09-11）：`JournalDetail` 为 `.journal-gallery figure` 注入 `--journal-gallery-index` 与 `--journal-gallery-tilt`，使用 `journal-gallery-photo-in` 完成图片画廊错峰入场，图片说明伪元素使用 `journal-gallery-ink` 绘制前置墨线；画廊保留来源、许可和懒加载语义，`prefers-reduced-motion` 下静态显示，路由、正文和内容数据不变。

> v0.3.136 发布补充（2026-09-04）：`NingxiaInteractiveMap` 在 `.map-viewport` 内、区域层之后渲染 `map-river-ink` SVG 路径，使用 `pathLength="1"` 与 `map-river-draw` 完成黄河水脉渐进绘图；路径为 `aria-hidden` 非交互装饰，随视口缩放/拖拽移动，`prefers-reduced-motion` 下静态显示，地图数据、图层、点位和交互逻辑不变。

> v0.3.135 发布补充（2026-09-04）：新增 `ReadingProgress` 全站阅读进度组件，使用 `requestAnimationFrame` 合并滚动/窗口变化更新，以 `--reading-progress` 驱动顶部墨线与笔头位置；不参与交互层，路由切换时重新测量，`prefers-reduced-motion` 下取消过渡但保留即时更新。
> v0.3.133 发布补充（2026-09-04）：站点头部移动端 `.mobile-nav a.active::after` 使用 `mobile-nav-active-ink`，页脚 `.footer-grid a.active::after` 使用 `footer-nav-active-ink`；两者均为非交互装饰，不改变导航路径、当前页语义、菜单焦点或滚动行为，`prefers-reduced-motion` 下静态显示。
> v0.3.132 发布补充（2026-09-04）：共用 `FavoriteButton` 在 `aria-pressed="true"` 时触发按钮 `::after` 的 `favorite-active-ink` 与图标 `favorite-heart-stamp`；两者均为非交互装饰，不改变收藏状态、跨页面同步、深色头图对比度或窄屏布局，`prefers-reduced-motion` 下静态显示。
> v0.3.129 发布补充（2026-09-04）：`Journal` 的当前栏目按钮使用 `journal-tab-active-ink` 在选中态底部展开轻量手绘线；墨线为 `::after` 非交互装饰，不改变栏目切换、URL 同步、键盘操作和窄屏横向浏览，`prefers-reduced-motion` 下静态显示。
> v0.3.128 发布补充（2026-09-04）：`NingxiaInteractiveMap` 在底图渐变与纹理之后、`.map-viewport` 之前渲染 5 条 `map-topography-ink` 椭圆墨线，使用 `pathLength="1"` 与 `map-topography-draw` 按索引错峰落笔；等高线属于 `aria-hidden` 非交互装饰，随地图视口移动，`prefers-reduced-motion` 下静态显示，地图数据、图层、点位和操作逻辑不变。
> v0.3.127 发布补充（2026-09-04）：`NingxiaInteractiveMap` 在地图画布包裹层渲染固定北向罗盘 `.map-compass`，使用 `map-compass-in` 完成首次进入的轻量落印；罗盘不属于 `.map-viewport`，因此不随拖拽/缩放移动，提供 `地图方向指示：上方为北` 无障碍名称，`prefers-reduced-motion` 下静态显示，地图点位、图层和视口逻辑不变。
> v0.3.126 发布补充（2026-09-04）：`RouteDetail` 为按天快速跳转链接注入 `--route-day-index`，激活链接使用 `route-day-active-ink` 沿底边绘制墨线；链接的锚点跳转、滚动激活、键盘访问与窄屏横向浏览逻辑不变，`prefers-reduced-motion` 下静态显示。
> v0.3.125 发布补充（2026-09-04）：`FoodLayer`、`GovernmentLayer` 与 `TransportLayer` 为可见点位渲染 `map-layer-ink-ring` SVG 圆环，使用 `pathLength="1"` 与 `map-layer-ink-ring-draw` 按索引错峰落印；圆环不参与交互，`prefers-reduced-motion` 下静态显示，图层开关、点位语义和地图层级逻辑不变。
> v0.3.124 发布补充（2026-09-04）：`AppRoutes` 使用当前 pathname 生成 `route-transition` 外层键，路由切换时为主内容统一触发 `route-page-in` 轻量入场；`prefers-reduced-motion` 下取消过渡并保持内容静态可见，滚动回顶、主要内容焦点、移动端菜单、筛选与深链接逻辑不变。
> v0.3.123 发布补充（2026-09-04）：`NingxiaInteractiveMap` 使用当前省级/城市/区县编码生成地图范围键，为 `MapRegionLayer`、`MapLabelLayer`、`AttractionLayer` 及已开启的美食、交通、政府图层提供范围级 `key`，复用 `map-region-draw`、`map-layer-pop-in` 和现有区域高亮动画完成层级切换重绘；`prefers-reduced-motion` 下维持静态显示，地图操作、点位选择、图例和预览逻辑不变。
> v0.3.122 发布补充（2026-09-04）：`AttractionsList` 与 `FoodsList` 使用规范化关键词和筛选组合生成结果键，结果摘要、卡片网格与空结果在筛选变化时重新挂载，复用 `collection-result-in`、`collection-card-in` 和现有状态动画完成结果重绘；`prefers-reduced-motion` 下维持静态显示，筛选、清除、收藏、对比和详情链接逻辑不变。
> v0.3.121 发布补充（2026-09-04）：`Search` 使用规范化关键词生成结果键，搜索结果区域在关键词变化时重新挂载，复用 `utility-summary-in`、`utility-group-in`、`utility-result-in`、`utility-empty-in` 和 `utility-suggestion-in` 完成分层重绘；`prefers-reduced-motion` 下维持静态显示，关键词提交、建议词、清空搜索、URL 和结果链接逻辑不变。
> v0.3.120 发布补充（2026-09-03）：`Journal` 使用栏目、城市、标签和搜索词生成结果键，结果摘要、文章网格与空结果在筛选变化时重新挂载，复用 `journal-result-in`、`journal-card-in`、`journal-card-title-ink` 和 `journal-state-in` 完成分层重绘；`prefers-reduced-motion` 下维持静态显示，URL、栏目键盘切换、清空筛选和链接逻辑不变。
> v0.3.119 发布补充（2026-09-03）：`RouteRecommendation` 使用筛选组合生成结果键，结果摘要、比较区域、路线网格与空结果在筛选变化时重新挂载，复用 `collection-result-in`、`collection-comparison-in`、`collection-table-row-in`、`collection-card-in` 和 `collection-chip-in` 完成分层重绘；`prefers-reduced-motion` 下维持静态显示，URL、筛选和链接逻辑不变。
> v0.3.118 发布补充（2026-09-03）：景点预览面板使用 `map-preview-photo-in`、`map-preview-content-in`、`map-preview-eyebrow-in`、`map-preview-title-ink`、`map-preview-copy-in` 和 `map-preview-action-in` 按图片、内容、位置、标题、摘要与行动入口分层绘图；预览通过景点 `key` 在切换时重新触发绘图，`prefers-reduced-motion` 下恢复静态内容与标题墨线，关闭、焦点和详情链接逻辑不变。
> v0.3.117 发布补充（2026-09-03）：地图区县图例使用 `map-legend-in`、`map-legend-title-in`、`map-legend-item-in` 和 `map-legend-swatch-in` 按面板、标题、条目与色块层级渐进绘图；图例通过 `key` 在城市切换时重新触发绘图，`prefers-reduced-motion` 下恢复静态图例，焦点联动与地图操作逻辑不变。
...
> 当前实现快照已更新至 v0.3.139：主题路线路书地理缩略图与按天流线墨线绘制、首页山河绘图分层入场、页面切换绘图、地图方向罗盘、地图底图等高线与黄河水脉、收藏状态心形落印与底部细墨线、旅行手记图片画廊错峰绘图与说明墨线、推荐路线筛选选中态墨线、景点兴趣筛选选中态墨线、旅行手记栏目激活墨线、地图叠加图层墨线落印、路线按天导航激活墨线、首页路线结果绘图、地图区域入场、地图视口过渡、路线轨道与节点落印、地图层级图层重绘、各类筛选结果重绘、全站搜索结果重绘、区域聚焦墨线、选中点位墨线和公共反馈层继续使用统一的响应式与动效约束；地图数据、搜索索引和内容核验机制不变。
...
| 测试 | Vitest + Playwright | 4.1 / 1.55 | 单元测试 + E2E |
| 图片处理 | sharp | 0.35.4 | 多格式图片生成 |
