...
> 当前实现快照已更新至 v0.3.129：首页山河绘图分层入场、页面切换绘图、地图方向罗盘、地图底图等高线、旅行手记栏目激活墨线、地图叠加图层墨线落印、路线按天导航激活墨线、首页路线结果绘图、地图区域入场、地图视口过渡、路线轨道与节点落印、地图层级图层重绘、各类筛选结果重绘、全站搜索结果重绘、区域聚焦墨线、选中点位墨线和公共反馈层继续使用统一的响应式与动效约束；地图数据、搜索索引和内容核验机制不变。
