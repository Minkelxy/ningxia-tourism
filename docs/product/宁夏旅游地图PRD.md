...
> v0.3.130 发布补充（2026-09-04）：景点“旅行兴趣”筛选的选中主题在标题下增加轻量手绘墨线，与旅行手记栏目、路线日程和导航状态共享纸面反馈语言；不改变筛选、URL 同步、结果卡片和窄屏布局，减少动效设置下恢复静态显示。
...
> 当前实现快照已更新至 v0.3.130：首页山河绘图分层入场、页面切换绘图、地图方向罗盘、地图底图等高线、景点兴趣筛选选中态墨线、旅行手记栏目激活墨线、地图叠加图层墨线落印、路线按天导航激活墨线、首页路线结果绘图、地图区域入场、地图视口过渡、路线轨道与节点落印、地图层级图层重绘、各类筛选结果重绘、全站搜索结果重绘、区域聚焦墨线、选中点位墨线和公共反馈层共享轻量动效基线，筛选输入、全站搜索、地图控制、收藏列表、景点卡片、路线详情和行前指南继续共享统一的响应式触控与布局基线。
