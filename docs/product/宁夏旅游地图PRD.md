...
> v0.3.74 发布补充（2026-09-02）：路线筛选按行程天数、涉及城市、旅行主题和行程节奏划分为四个可识别分组，为辅助技术提供独立名称；不改变筛选布局、地址参数或结果展示。
...
> 当前实现快照已更新至 v0.3.74：路线筛选四组条件拥有独立语义名称；地图懒加载占位与正式画布共享沙纸色、纹理和抽象轮廓；桌面端路径变化后焦点进入唯一的 `main#main-content`，移动端菜单跳转保留菜单按钮焦点；其余视觉、触控热区和内容核验基线不变。
