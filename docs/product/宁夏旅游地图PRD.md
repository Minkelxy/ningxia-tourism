...
> v0.3.139 发布补充（2026-09-23）：修正路书图片导出的竞态。省界底图是异步加载的，原先在独立路书页点击「生成图片」即序列化海报，会在省界到达前导出只有路线、没有地图底图的残缺图片，且不给任何提示；现等省界落定后再序列化，与路线详情页的导出门禁一致。同时清理路书地理图中永不生效的兜底配色分支、把海报折行的 React key 改为索引，并补一条端到端断言守住该回归。不改变路线停靠点事实、核实等级、内容数据模型与既有链接／文案分享。
> v0.3.138 发布补充（2026-09-22）：主题路线新增可视化路书。路线详情页在时间线之前提供「线路一览」区块，左侧为按天分段连线的地理缩略图，右侧为按天流线，编号跨天连续；新增独立路书页 `/routes/:routeId/roadbook`，页面即一张竖版海报加一份等价文字版路书，可导出 PNG 图片或打印成纸质路书。没有坐标的市区／车站类查询点只出现在流线与图例中，不参与地图连线。不改变路线停靠点事实、核实等级、内容数据模型与既有链接／文案分享。
> v0.3.137 发布补充（2026-09-11）：旅行手记详情页图片画廊新增按阅读顺序错峰入场、轻微交错倾角与图片说明手绘墨线，与封面、卡片和标题墨线共享纸面视觉语言；保留图片来源、许可和懒加载语义，减少动效设置下恢复静态显示，不改变内容事实、路由和阅读结构。

> v0.3.136 发布补充（2026-09-04）：交互地图新增低对比度黄河水脉 SVG 绘图，沿地图视口渐进落笔并随缩放、拖拽移动；作为非交互装饰补足山河主题，减少动效设置下恢复静态显示，不改变地图数据、图层、点位和操作逻辑。

> v0.3.135 发布补充（2026-09-04）：全站长页面滚动时沿顶部实时绘制阅读进度墨线，以轻量笔头提示阅读位置；不改变页面内容、滚动回顶、锚点跳转或打印版布局，减少动效设置下即时更新。
> v0.3.133 发布补充（2026-09-04）：移动端导航菜单与页脚当前入口增加统一激活墨线，与桌面主导航共享纸面状态反馈语言；不改变导航路径、当前页语义、移动端菜单焦点与滚动行为，减少动效设置下恢复静态显示。
> v0.3.132 发布补充（2026-09-04）：全站收藏按钮切换为已收藏时增加轻量心形落印与底部手绘墨线，与筛选、旅行手记栏目和路线日程共享纸面状态反馈语言；不改变收藏逻辑、跨页面同步、深色头图对比度和窄屏图标化布局，减少动效设置下恢复静态显示。
...
> 当前实现快照已更新至 v0.3.139：主题路线路书地理缩略图与按天流线墨线绘制、首页山河绘图分层入场、页面切换绘图、地图方向罗盘、地图底图等高线与黄河水脉、收藏状态心形落印与底部细墨线、旅行手记图片画廊错峰绘图与说明墨线、推荐路线筛选选中态墨线、景点兴趣筛选选中态墨线、旅行手记栏目激活墨线、地图叠加图层墨线落印、路线按天导航激活墨线、首页路线结果绘图、地图区域入场、地图视口过渡、路线轨道与节点落印、地图层级图层重绘、各类筛选结果重绘、全站搜索结果重绘、区域聚焦墨线、选中点位墨线和公共反馈层共享轻量动效基线，筛选输入、全站搜索、地图控制、收藏列表、景点卡片、路线详情和行前指南继续共享统一的响应式触控与布局基线。
