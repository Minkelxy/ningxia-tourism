...
test('桌面端页面跳转后使用统一的轻量入场过渡', async ({ page }) => {
  if ((page.viewportSize()?.width ?? 999) <= 768) test.skip();
  await page.goto(appBase);
  await page.locator('header').getByRole('link', { name: '精选景点' }).click();
  await expect(page).toHaveURL(/\/attractions$/);
  await expect(page.locator('.route-transition')).toHaveCSS('animation-name', 'route-page-in');
  await expect(page.locator('.route-transition')).toHaveCSS('animation-duration', '0.32s');
});

test('页面入场过渡在减少动效时恢复静态显示', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.goto(`${appBase}attractions`);
...
test('首页山河绘图与地图区域动效保持轻量并尊重减少动效设置', async ({ page }) => {
  ...
  const river = map.locator('.map-river-ink');
  ...
  await expect(river).toHaveCSS('animation-name', 'map-river-draw');
  await expect(river).toHaveAttribute('pathLength', '1');
  ...
  await compass.evaluate(async (element) => {
  ...
  const movedCompassBox = await compass.boundingBox();
  expect(movedCompassBox?.x).toBeCloseTo(compassBox?.x ?? -1, 1);
  expect(movedCompassBox?.y).toBeCloseTo(compassBox?.y ?? -1, 1);

  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.reload();
  await page.locator('.lazy-map-container').scrollIntoViewIfNeeded();
  const reducedMotion = await page.locator('.sun-disc, .mountain-back, .mountain-front, .hero-seal, .scroll-cue, .map-compass, .map-topography-ink, .map-river-ink, .map-region, .map-region-emphasis, .map-attraction-glyph, .map-label__glyph').evaluateAll((elements) => elements.map((element) => getComputedStyle(element).animationName));
