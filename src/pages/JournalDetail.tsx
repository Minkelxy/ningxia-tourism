...
import { ArrowLeft, CalendarDays, CircleDollarSign, Clock3, MapPin, Navigation, NotebookPen, Share2 } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { Link, useParams } from 'react-router-dom';
import SEO from '../components/SEO';
import ResponsiveImage from '../components/ResponsiveImage';
import { getJournalEntry } from '../content/journal';
...
export default function JournalDetail() {
  ...
  if (!entry || entry.status !== 'published' || entry.contentKind !== 'firsthand') return <div className="full-state journal-missing"><SEO title="手记整理中 · 宁夏旅行地图" noIndex /><NotebookPen aria-hidden="true" /><p className="eyebrow">内容整理中</p><h1>这篇手记还没有公开</h1><p>真实经历和照片完成核对后才会发布。先回到旅行手记查看栏目说明。</p><Link to="/journal" className="btn-primary">返回旅行手记</Link></div>;
  ...
  return <>
    <SEO title={`${entry.title} · 旅行手记`} description={entry.excerpt} image={entry.cover.src} type="article" publishedAt={entry.publishedAt} updatedAt={entry.updatedAt} author={entry.author} />
    <div className="journal-detail"><header className="journal-detail-hero"><ResponsiveImage src={entry.cover.src} alt={entry.cover.alt} width="1600" height="960" sizes="100vw" /><div className="detail-overlay" /><div className="section-shell journal-detail-title"><Link to="/journal" className="back-link"><ArrowLeft aria-hidden="true" />旅行手记</Link><p className="eyebrow">{entry.type === 'travel' ? '个人游记' : '探店记录'} · {entry.author}</p><h1>{entry.title}</h1><p>{entry.excerpt}</p><button type="button" className="icon-button" onClick={handleShare} aria-label="分享手记"><Share2 aria-hidden="true" /></button></div></header>
      {shareStatus && <div className="toast" role="status">{shareStatus}</div>}
      <div className="section-shell journal-detail-layout"><article className="journal-article"><div className="journal-fact-strip"><span><CalendarDays aria-hidden="true" />{travel?.tripDate || food?.visitedAt}</span><span><MapPin aria-hidden="true" />{cityName(entry.cityId)} · {entry.locality}</span>{travel && <span><Clock3 aria-hidden="true" />{travel.duration}</span>}{food && <span><CircleDollarSign aria-hidden="true" />{food.pricePerPerson}</span>}</div>
        ...
        {entry.gallery.length > 0 && <section className="journal-gallery">{entry.gallery.map((image) => <figure key={image.src}><ResponsiveImage src={image.src} alt={image.alt} loading="lazy" /><figcaption>{image.credit} · {image.license}</figcaption></figure>)}</section>}
      </article><aside className="journal-sidebar"><div className="date-stamp">{entry.publishedAt.slice(0, 7).replace('-', '.')}<br />宁夏</div>{headings.length > 0 && <nav className="journal-toc" aria-label="正文目录"><h2>这篇写了什么</h2>{headings.map((heading) => <a key={`${heading.level}-${heading.id}`} className={heading.level === 3 ? 'sub' : ''} href={`#${heading.id}`}>{heading.text}</a>)}</nav>}<a className="btn-quiet journal-map-link" href={mapUrl} target="_blank" rel="noreferrer"><Navigation aria-hidden="true" />高德查看地点</a><div className="journal-change-note"><h2>关于时效</h2><p>{food ? '这是一次到店当日的个人体验，不代表商户当前价格、营业时间或长期水平。' : '交通、开放和消费信息会变化，文章只代表当次旅行条件。'}</p></div>{entry.relatedAttractionIds.length > 0 && <div className="related-notes"><h2>相关景点</h2>{entry.relatedAttractionIds.map((id) => { const item = getAttractionById(id); return item && <Link key={id} to={`/attraction/${id}`}>{item.name}</Link>; })}</div>}{entry.relatedRouteIds.length > 0 && <div className="related-notes"><h2>相关路线</h2>{entry.relatedRouteIds.map((id) => { const route = getRouteById(id); return route && <Link key={id} to={`/routes/${id}`}>{route.name}</Link>; })}</div>}<div className="image-credit"><span>封面图片</span><a href={entry.cover.sourceUrl} target="_blank" rel="noreferrer">{entry.cover.credit} · {entry.cover.license}</a></div></aside></div>
    </div>
  </>;
}
