...
export default function CityOverview() {
  ...
  if (city) {
    return (
      <>
        ...
        <div>
          <header className="city-detail-hero"><ResponsiveImage src={city.image.src} alt={city.image.alt} loading="eager" fetchPriority="high" width="1600" height="960" sizes="100vw" /><div className="detail-overlay" /><div className="city-detail-copy"><span className="detail-photo-badge">实景资料</span><Link to="/cities" className="back-link"><ArrowLeft aria-hidden="true" /> 五城概览</Link><p className="eyebrow">{city.nickname}</p><h1>{city.name}</h1><p>{city.introduction}</p></div></header>
  ...
  return (
    <>
      ...
      <header className="page-hero compact-hero collection-hero"><div className="section-shell collection-hero-grid"><div><p className="eyebrow"><MapPin aria-hidden="true" /> 五座城市 · 五种旅行气质</p><h1>宁夏不只有沙漠</h1><p>先比较停留时间、适合人群与交通衔接，再选择一座城市，理解它的历史、味道与旅行节奏。</p></div><div className="collection-hero-visual"><ResponsiveImage src="/images/ai-samples/city-constellation.webp" alt="AI 生成的宁夏五城城市群、铁路与山河主题插画，仅作城市概览氛围示例" width="720" height="480" loading="eager" fetchPriority="high" sizes="(max-width: 768px) 100vw, 42vw" /><span>AI 视觉示例 · 非实景</span></div></div></header>
