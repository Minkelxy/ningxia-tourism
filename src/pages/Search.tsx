import { ArrowRight, Compass, MapPin, Search as SearchIcon, Sparkles, UtensilsCrossed, X } from 'lucide-react';
...
export default function Search() {
  ...
  return <>
    ...
    <header className="search-hero"><div className="section-shell search-hero-grid">
      <div><p className="eyebrow"><SearchIcon aria-hidden="true" /> 全站搜索</p><h1>从一个关键词，<br />找到下一站。</h1><form className="site-search-form" onSubmit={submit} aria-label="搜索宁夏旅行内容"><div className="site-search-field"><label htmlFor="site-search-input"><SearchIcon aria-hidden="true" /><span className="sr-only">搜索宁夏旅行内容</span></label><input id="site-search-input" ref={inputRef} value={input} onChange={(event) => setInput(event.target.value)} placeholder="试试：沙漠、早茶、三天、六盘山" />{input && <button type="button" className="search-clear-button" onClick={clearSearch} aria-label="清空搜索内容" title="清空搜索内容"><X aria-hidden="true" /></button>}</div><button type="submit" className="btn-primary">搜索 <ArrowRight aria-hidden="true" /></button></form><p className="search-hint">可搜索景点、美食、路线、城市名、旅行专题和标签。</p></div>
