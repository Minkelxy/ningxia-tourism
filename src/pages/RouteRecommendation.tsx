import { ArrowRight, CalendarDays, CircleDollarSign, Compass, Sparkles } from 'lucide-react';
  ...
  return (
    <>
      ...
      <main className="section-shell page-content">
        <section className="route-filter" aria-label="路线筛选"><div><span>行程天数</span>{['all', '1', '2', '3', '4+'].map((value) => <button type="button" key={value} className={duration === value ? 'active' : ''} onClick={() => setFilter('duration', value)}>{value === 'all' ? '全部' : value === '4+' ? '4 天以上' : `${value} 天`}</button>)}</div><div><span>旅行主题</span><button type="button" className={theme === 'all' ? 'active' : ''} onClick={() => setFilter('theme', 'all')}>全部</button>{Object.entries(routeThemeLabels).map(([value, label]) => <button type="button" key={value} className={theme === value ? 'active' : ''} onClick={() => setFilter('theme', value)}>{label}</button>)}</div></section>
        <div className="result-summary"><strong>{filtered.length}</strong> 条路线</div>
        <div className="route-grid">{filtered.map((route, index) => <article className={`route-card route-tone-${index % 4}`} key={route.id}><div className="route-card-top"><span>{route.themeLabel}</span><strong>{route.durationLabel}</strong></div><h2>{route.name}</h2><p>{route.summary}</p><div className="route-highlights">{route.highlights.map((item) => <span key={item}>{item}</span>)}</div><dl><div><CalendarDays aria-hidden="true" /><dt>适合</dt><dd>{route.audience}</dd></div><div><CircleDollarSign aria-hidden="true" /><dt>预算</dt><dd>{route.budget}</dd></div><div><Compass aria-hidden="true" /><dt>季节</dt><dd>{route.bestSeason}</dd></div></dl><Link to={`/routes/${route.id}`} className="text-link">查看逐日安排 <ArrowRight aria-hidden="true" /></Link></article>)}</div>
