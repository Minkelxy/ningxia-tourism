...
export default function RouteRecommendation() {
  ...
  const activeFilterCount = [duration !== 'all', theme !== 'all', pace !== 'all', city !== 'all'].filter(Boolean).length;
  const [filtersExpanded, setFiltersExpanded] = useState(() => activeFilterCount > 0);
  const filtered = routes.filter((route) => (duration === 'all' || (duration === '4+' ? route.durationDays >= 4 : route.durationDays === Number(duration)))
    && (theme === 'all' || route.theme === theme)
    && (pace === 'all' || route.pace === pace)
    && (city === 'all' || getRouteEvidenceSummary(route).cityIds.includes(city as CityId)));
  ...
  return (
    <>
      ...
      <header className="route-hero compact-hero"><div className="section-shell"><p className="eyebrow"><Sparkles aria-hidden="true" /> {routes.length} 条经过整理的主题路线</p><h1>先决定有几天，<br />再决定怎么认识宁夏。</h1><p>路线不是必须照抄的清单，而是一套减少折返、保留弹性的地理顺序。票价、班次和营业安排请在出发前再次确认。</p></div></header>
      <div className="section-shell page-content">
        <div className="mobile-filter-bar route-mobile-filter-bar"><button type="button" className="mobile-filter-toggle" aria-expanded={filtersExpanded} aria-controls="route-filters" onClick={() => setFiltersExpanded((current) => !current)}><SlidersHorizontal aria-hidden="true" /><span>筛选路线</span>{activeFilterCount > 0 && <strong aria-label={`${activeFilterCount} 个筛选条件`}>{activeFilterCount}</strong>}{filtersExpanded ? <ChevronUp aria-hidden="true" /> : <ChevronDown aria-hidden="true" />}</button><a href="#route-results">{filtered.length} 条结果</a></div>
        ...
        <div id="route-results" className="result-summary" role="status" aria-live="polite"><span><strong>{filtered.length}</strong> 条路线</span>{city !== 'all' && <span className="active-filter-note">城市：{cities.find((item) => item.id === city)?.name}</span>}{hasFilters && <button type="button" onClick={() => { setParams({}); setFiltersExpanded(false); }}>清除筛选</button>}</div>
