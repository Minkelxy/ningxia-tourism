...
function TransportLayer({ hubs, project }: TransportLayerProps) {
  return hubs.map((hub) => {
    ...
    return <g key={hub.id} className={hubClassName} transform={`translate(${point.x} ${point.y})`} role="img" aria-label={`${hub.name}，${hub.type === 'airport' ? '机场' : hub.type === 'bus' ? '客运站' : '火车站'}交通枢纽`}><circle r="11" /><Icon x={-7} y={-7} width={14} height={14} /><title>{tooltip}</title></g>;
  });
}
