...
涉及页面结构时，请保持应用外壳提供的唯一 `<main id="main-content">`；页面组件内部使用 `<div>` 或语义化 section，不要再嵌套第二个 `<main>`。涉及搜索、收藏、错误状态或开发工具页时，至少补一条“页面仅有一个主内容区域”的端到端回归。路径变化后桌面端应将焦点交给新的 `main#main-content`，移动端菜单跳转应保留菜单按钮焦点；带输入框的页面可为桌面端提供自动聚焦，但移动端应避免进入页面即唤起系统键盘，并分别覆盖两种端型的焦点状态。搜索词或筛选参数变化时，也要保留页面状态播报；同一路径参数更新不得依赖整页跳转才能被辅助技术感知。景点、美食、首页专题卡片、城市/路线比较表名称入口以及路线详情的停靠点名称入口都必须保持至少 44px 触控高度；比较表的“查看”操作入口还必须保持至少 44px 宽，手记栏目标签在 320px 等窄屏下必须可横向浏览；关于页指向仓库内文档的链接必须与实际路径一致，并覆盖桌面与移动端回归。
...
- 可交互地图点位的可视标记可以保持紧凑，但必须提供不小于 44px 视觉触控范围；键盘聚焦时应使用与景点点位一致的胡杨绿状态反馈，纯展示点位不得加入透明交互热区。
...
新增功能或 UI 变更时，应同步补充对应测试用例。路线详情的日程锚点支持直接访问；调整路由初始化、懒加载或滚动逻辑时，需回归 `#route-day-N` 深链接在桌面端与移动端的定位，并确认目标标题位于站点头部与按天吸顶导航之下；调整 pathname 焦点管理时，需同时回归普通桌面导航跳转与移动端菜单按钮焦点回收。

### 导航语义

- 当前页面的主导航、搜索和收藏入口需同时保持视觉选中态与 `aria-current="page"` 语义。
- 页脚“继续探索”应与顶部主导航保持景点、美食、路线、指南、手记和五城六类内容入口一致；入口列表与路径匹配统一维护在 `src/lib/site-navigation.ts`，页脚分组应使用带 `aria-label` 的导航语义，当前入口需同时保持选中态与 `aria-current="page"`，资料说明与建议入口单独分组。景点、美食、首页专题卡片以及城市/路线比较表的名称入口都必须保持至少 44px 触控高度。
...
- 320px 等极窄屏宽度下，品牌主名不得换行挤压搜索、收藏和菜单入口；可以隐藏重复副标题，但需保留完整的无障碍品牌名称，并回归页面不得横向溢出。
- 含宽表格的移动端页面必须让横向滚动限制在表格容器内；回归时同时检查容器仍可滚动，以及 `document.documentElement.scrollWidth` 不超过视口宽度。
- 路线详情的站点头部、按天导航、日程锚点和 DAY 标记必须使用同一套响应式高度变量；修改任一吸顶层高度时，需同步检查 375—480px 手机宽度的跳转安全区。
- 路线详情深色头图中的打印、分享与收藏操作必须使用同一套半透明按钮层级；收藏仍需保留 `aria-pressed`、已收藏状态和键盘聚焦反馈，桌面与移动端都要回归默认、悬停和聚焦样式。
- 详情页返回入口保留 44px 触控高度；悬停和键盘聚焦只移动左侧箭头，不整体移动链接热区。
- 移动端菜单按钮保持 44px 触控尺寸；悬停和键盘聚焦需使用胡杨绿、轻量抬升与阴影反馈，与搜索和收藏入口节奏一致。
- 移动端展开菜单的未选中项也必须提供与桌面导航一致的悬停/键盘聚焦背景、主题色和阴影反馈；若菜单有入场动效，E2E 应在动效稳定后再断言样式。
- 移动端菜单打开时应使用实色悬浮面板，不得推移页面首屏或让底层文字透出；菜单外需有轻量遮罩并允许点击关闭，背景滚动需锁定，点击菜单入口后应立即收起并将焦点回收到菜单按钮，关闭、Escape 或路由切换后必须恢复原有设置，同时保留菜单内焦点循环与点击外部关闭。
- 移动端地图景点预览使用固定底部面板时，应保留安全区、可见面板把手和 reduced-motion 友好的入场动效；把手仅作装饰并标记为 `aria-hidden`，关闭按钮、Escape 和关闭后焦点回收行为不得改变。
- 地图景点预览的入场动效应在桌面端和移动端保持同一命名与视觉节奏；移动端可以覆盖时长和位移，但不得取消 `map-preview-in` 或破坏固定面板的安全区。
- 路由懒加载的公共等待状态应沿用品牌印章、主题色和统一圆角语言，并提供明确的 `role="status"` / 可访问名称；地图懒加载占位还应与正式地图画布保持同源的沙纸色、纹理和轮廓语言；动画必须服从 reduced-motion 设置。
- 固定或吸顶的公共反馈层（如收藏提示、PWA 更新提示、对比停靠条）必须使用 `env(safe-area-inset-bottom, 0px)` 预留移动端底部手势区，同时保持桌面端原有间距与层级。
- 移动端英雄区不得使用未经内容需要证明的超大底部留白；若为视觉装饰保留额外空间，必须在 320—480px 宽度检查首个主要内容区的可见节奏，并同步保留桌面端原有间距。
