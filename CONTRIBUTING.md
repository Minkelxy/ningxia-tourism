...
- 页脚“继续探索”应与顶部主导航保持景点、美食、路线、指南、手记和五城六类内容入口一致；入口列表与路径匹配统一维护在 `src/lib/site-navigation.ts`，页脚分组应使用带 `aria-label` 的导航语义，当前入口需同时保持选中态与 `aria-current="page"`，资料说明与建议入口单独分组。景点、美食和首页专题卡片的标题入口也必须保持至少 44px 触控高度。
