...
涉及页面结构时，请保持应用外壳提供的唯一 `<main id="main-content">`；页面组件内部使用 `<div>` 或语义化 section，不要再嵌套第二个 `<main>`。涉及搜索、收藏、错误状态或开发工具页时，至少补一条“页面仅有一个主内容区域”的端到端回归。带输入框的页面可为桌面端提供自动聚焦，但移动端应避免进入页面即唤起系统键盘，并分别覆盖两种端型的焦点状态。搜索词或筛选参数变化时，也要保留页面状态播报；同一路径参数更新不得依赖整页跳转才能被辅助技术感知。景点、美食、首页专题卡片、城市/路线比较表以及路线详情的停靠点名称入口都必须保持至少 44px 触控高度，并覆盖桌面与移动端回归。
