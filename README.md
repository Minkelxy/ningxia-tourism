...
一个面向国内游客的宁夏静态旅行规划站。网站用自研 SVG 地图串起 5 个地级市、12 个已核实景点和 7 条主题路线，并保留来源、核实日期与图片许可信息。
...
正式景点必须补齐开放信息、参考票价、预约、时长、季节、交通、WGS84 坐标、来源、核实日期和图片许可。资料无法确认时应保留为 `draft`，不应猜测补全。

## 路由与部署

公开路由包括 `/`、`/attractions`、`/attraction/:id`、`/cities`、`/city/:slug`、`/routes`、`/routes/:routeId` 和 `/about`。GeoJSON 工具只在开发环境注册。
