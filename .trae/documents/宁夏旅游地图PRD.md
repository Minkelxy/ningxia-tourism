...
#### 2.2.4 美食目录
- **14 道特色美食**: 手抓羊肉、蒿子面、八宝茶、羊杂碎、辣糊糊等
...
#### 2.2.7 旅行手记
- **资料型专题**: 10 篇已公开，覆盖五个地级市，含来源、核对日期和适用范围
...
1. **阶段一**: 完成文档和设计稿
2. **阶段二**: 实现核心地图交互功能（SVG 地图 + 景点图层）
3. **阶段三**: 完成景点详情页面
4. **阶段四**: 添加城市概览、路线推荐和路线详情
5. **阶段五**: 添加美食目录与美食详情页，地图美食图层联动
6. **阶段六**: 添加旅行手记（资料专题）与搜索筛选
7. **阶段七**: 实现数据校验门禁（verifiedAt 时效、来源分层、反糟粕）
8. **阶段八**: 地图图层无障碍统一、React.memo 性能优化
9. **阶段九**: 添加行前指南、关于页面和首页路线匹配
10. **阶段十**: 测试、性能门禁和持续内容复核
